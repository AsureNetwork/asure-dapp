// From https://github.com/etherisc/tokensale/blob/master/util/logger.js
...
const getTime = () => {
  ...
  return `${padL(now.getHours())}:${padL(now.getMinutes())}:${padL(
    now.getSeconds()
  )}.${padR(now.getMilliseconds())}`; // eslint-disable-line
};
...
const colorAddress = message => {
  ...
  if (regex.test(message)) {
    return message.replace(
      new RegExp(regex),
      chalk.yellow(message.match(regex)[0])
    );
  }
